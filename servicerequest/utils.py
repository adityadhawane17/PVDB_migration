...
class Utils:
    ...
    def get_max_value(self, columnname, tablename, dbconfigname):
        ...
        conn = None
        query="select max({columnname}) from {tablename};".format(columnname=columnname,tablename=tablename)
        print(query)
        try:
            params = dbconfig(filename="database.ini", section=dbconfigname)
            conn = psycopg2.connect(**params)
    ...
    def fetch_paginated_records(self,sourcename,pk_name, per_page_records,outputfolder,dbconfigname,columnnames,sql_file_location):
        self.logging.info("Start fetching paginated records for {sourcename}".format(sourcename=sourcename))
        primary_key_id=0
        max_primary_key_id=self.get_max_value(pk_name,sourcename,dbconfigname)
        print(max_primary_key_id)
        ...
        while (primary_key_id < max_primary_key_id):
